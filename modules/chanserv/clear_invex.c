...
static void cs_cmd_clear_invex(sourceinfo_t *si, int parc, char *parv[])
{
	...
	{
		if (changes > 1)
			verbose(mc, "\2%s\2 removed all %d channel invite exceptions.",
				get_source_name(si), changes);
		else
			verbose(mc, "\2%s\2 removed 1 channel invite exception.",
				get_source_name(si));
	}
	...
	if (add_history_entry != NULL)
	{
		...
		snprintf(desc, sizeof desc, "Cleared all invite exceptions (removed %d).", changes);
