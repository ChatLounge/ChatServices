 ...
 *
 * This file contains code for the CService CLEAR AKICKS function.
 *
 */
...
static void cs_cmd_clear_akicks(sourceinfo_t *si, int parc, char *parv[])
{
	...
	command_success_nodata(si, _("Cleared AKICK entries on: \2%s\2"), mc->name);
	if (changes > 0)
		verbose(mc, _("\2%s\2 cleared the AKICK list, removing %d AKICK (+b) entr%s."),
	...
	if (add_history_entry != NULL)
	{
		...
		snprintf(desc, sizeof desc, "Cleared all AKICKS (removed %d).", changes);
